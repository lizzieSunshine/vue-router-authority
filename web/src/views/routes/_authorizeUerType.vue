<template>
  <el-dialog class="auth-page" :title="`${routeName} · 用户类型授权`" :visible.sync="visible">
    <el-alert class="alert-info" type="warning" :closable="false">
      请为路由指定用户类型，如不选择，默认该路由无权限控制
      <el-button type="text">
        <router-link :to="{ name: 'enums', query: { path: $route.query.p } }"><i class="el-icon-edit"></i>用户类型</router-link>
      </el-button>
    </el-alert>
    {{roles}}
    <el-form :model="form">
